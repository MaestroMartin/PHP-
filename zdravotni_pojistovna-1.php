...
do {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $pdf->setSourceFile('C:/xampp/htdocs/tabor/templates/dokument1.pdf'); // cesta k tvojí šabloně PDF
       ...
       // Příjmení a jméno
       $pdf->SetXY(90, 50); // souřadnice pole 
       $pdf->Write(0, $row["jmeno"] . " " . $row["prijmeni"]);

       $pdf->SetXY(80, 130); // Nastavení pozice pro kontaktní adresu
       $kontakni_adresa = "Jitřenka Bučovice, Žandovský mlýn, 345 Archlebov, 696 33";
       $pdf->MultiCell(60, 10, $kontakni_adresa, 0, 'L', 0);  // MultiCell s šířkou 60, výškou 10, zarovnáním doleva


       // Číslo pojištěnce (rodné číslo)
       $pdf->SetXY(x: 155, y: 50); // souřadnice pole 
       $pdf->Write(0, substr($row["cislo_pojistence"], 0, 12));
        // Název souboru
        $directory = 'C:/xampp/htdocs/tabor/';
        $jmeno_souboru = $directory . str_replace(' ', '_', $row["prijmeni"] . "_" . $row["jmeno"] ."_ZP". ".pdf");
