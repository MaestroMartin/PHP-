...
do {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $pdf->setSourceFile('C:/xampp/htdocs/tabor/templates/dokument2.pdf'); // cesta k tvojí šabloně PDF
        ...
        // Příjmení a jméno
        $pdf->SetXY(120, 70); // souřadnice pole 
        $pdf->Write(0, $row["jmeno"] . " " . $row["prijmeni"]);

        // Číslo pojištěnce (rodné číslo)
        $pdf->SetXY(120, 85); // souřadnice pole 
        $pdf->Write(0, substr($row["cislo_pojistence"], 0, 12));
        
        $pdf->SetXY(120, 190); // Nastavení pozice pro kontaktní adresu
        ...
        $jmeno_souboru = $directory . str_replace(' ', '_', $row["prijmeni"] . "_" . $row["jmeno"] . "_CPP" . ".pdf");
